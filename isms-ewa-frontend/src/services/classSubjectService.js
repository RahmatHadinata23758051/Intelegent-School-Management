...
const BASE_URL = '/api/class-subjects';
...
export const classSubjectService = {
  ...
  getSubjectsByClass: async (classId, params = {}) => {
    const response = await api.get(`/api/classes/${classId}/subjects`, { params });
    return response.data;
  },
  ...
  getClassesBySubject: async (subjectId, params = {}) => {
    const response = await api.get(`/api/subjects/${subjectId}/classes`, { params });
    return response.data;
  },
